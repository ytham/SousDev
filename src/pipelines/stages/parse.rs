...
#[error("SkipPipelineSignal: parser returned None — nothing to process this tick")]
pub struct SkipPipelineSignal;
...
/// - `None` — skip this run (a [`SkipPipelineSignal`] error is returned).
    ...
    F: Fn(&str) -> Option<ParsedTask> + Send + Sync,
{
    ...
    async fn run(&self, ctx: &mut StageContext) -> Result<()> {
        ...
        match (self.parser)(stdout) {
            ...
            None => Err(SkipPipelineSignal.into()),
        }
    }
}
...
mod tests {
    use super::*;
    use crate::pipelines::stage::{ResolvedPrompts, StageContext};
    ...
    async fn test_parse_none_returns_skip_signal() {
        ...
        // Verify it downcasts to SkipPipelineSignal.
        assert!(
            err.downcast_ref::<SkipPipelineSignal>().is_some(),
            "expected SkipPipelineSignal, got: {}",
            err
        );
    }
