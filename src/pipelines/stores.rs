...
/// The result of a single pipeline run, persisted by [`RunStore`].
...
/// Summary of a PR review pass stored inside a [`PipelineResult`].
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Summary of a PR response pass stored inside a [`PipelineResult`].
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Append-only store for [`PipelineResult`] records, written to
...
impl RunStore {
    ...
    pub async fn get_history(
        ...
        let filtered: Vec<PipelineResult> = match pipeline_name {
            Some(name) => all.into_iter().filter(|r| r.pipeline_name == name).collect(),
            None => all,
        };
...
/// A record written when an issue has been successfully handled by a pipeline.
...
/// Tracks which issues each pipeline has already handled, preventing duplicate
/// processing across cron ticks.
///
/// Persisted to `.harness-handled-issues.json`.  The file schema is
/// `{ pipeline_name: { issue_number_str: HandledIssueRecord } }`.
...
impl HandledIssueStore {
    ...
    /// Return `true` when `pipeline_name` has already handled `issue_number`.
    pub async fn is_handled(&self, pipeline_name: &str, issue_number: u64) -> Result<bool> {
    ...
    pub async fn mark_handled(
        ...
        data.entry(pipeline_name.to_string())
            .or_default()
    ...
    pub async fn unmark_handled(&self, pipeline_name: &str, issue_number: u64) -> Result<()> {
        let mut data = self.read_all().await.unwrap_or_default();
        if let Some(m) = data.get_mut(pipeline_name) {
...
/// A record written when a pipeline has reviewed a pull request.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Tracks which PRs each pipeline has reviewed and at which HEAD SHA, enabling
...
impl PrReviewStore {
    ...
    /// Retrieve the stored review record for `pr_number` under `pipeline_name`,
    ...
    /// Upsert a review record for `pipeline_name`.
    ...
    /// Remove the review record for `pr_number` under `pipeline_name`.
    pub async fn remove_record(&self, pipeline_name: &str, pr_number: u64) -> Result<()> {
        let mut data = self.read_all().await.unwrap_or_default();
        if let Some(m) = data.get_mut(pipeline_name) {
...
/// A record written when a pipeline has responded to review comments on a PR.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Tracks which PRs each pipeline has already responded to, and the cursor
...
impl PrResponseStore {
    ...
    /// `pipeline_name`, returning `None` if no record exists.
    pub async fn get_record(
        &self,
        pipeline_name: &str,
        pr_number: u64,
    ...
    /// Upsert a response record for `pipeline_name`.
    ...
    /// Remove the response record for `pr_number` under `pipeline_name`.
    pub async fn remove_record(&self, pipeline_name: &str, pr_number: u64) -> Result<()> {
        let mut data = self.read_all().await.unwrap_or_default();
        if let Some(m) = data.get_mut(pipeline_name) {
            m.remove(&pr_number.to_string());
...
mod tests {
    ...
    #[tokio::test]
    async fn test_run_store_filters_by_pipeline() {
        let dir = TempDir::new().unwrap();
        let store = RunStore::new(dir.path());
        for name in &["pipe-a", "pipe-b", "pipe-a"] {
            store
                .append(&PipelineResult {
    ...
    #[tokio::test]
    async fn test_handled_issue_store_cross_pipeline_isolation() {
        let dir = TempDir::new().unwrap();
        let store = HandledIssueStore::new(dir.path());
        store
            .mark_handled_with_number(
    ...
    #[tokio::test]
    async fn test_pr_response_store_cross_pipeline_isolation() {
        let dir = TempDir::new().unwrap();
        let store = PrResponseStore::new(dir.path());
        store
            .mark_responded(
    ...
    #[tokio::test]
    async fn test_run_store_get_pipeline_names() {
        let dir = TempDir::new().unwrap();
        let store = RunStore::new(dir.path());
        ...
        let names: std::collections::HashSet<&str> =
            all.iter().map(|r| r.pipeline_name.as_str()).collect();
    ...
    async fn test_run_store_append_preserves_all_fields() {
        ...
        let store = RunStore::new(dir.path());
        let result = PipelineResult {
            pipeline_name: "full-pipeline".into(),
            run_id: "run-999".into(),
        ...
        assert_eq!(history.len(), 1);
        let r = &history[0];
        assert_eq!(r.pipeline_name, "full-pipeline");
    ...
    #[tokio::test]
    async fn test_handled_issue_store_multiple_issues_same_pipeline() {
        let dir = TempDir::new().unwrap();
        let store = HandledIssueStore::new(dir.path());
        for n in 1..=3 {
            store
    ...
    #[tokio::test]
    async fn test_pr_review_store_cross_pipeline_isolation() {
        let dir = TempDir::new().unwrap();
        let store = PrReviewStore::new(dir.path());
        store
            .mark_reviewed(
