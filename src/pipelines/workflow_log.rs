 ...
 * `output/logs/<pipeline_name>/<run_id>.json`.
 *
 * Each pipeline run gets its own log file containing timestamped entries
 * with a level, stage name, and message.  This enables a TUI to:
 *   - List past runs per pipeline (read the directory)
 *   - Stream or page through a run's log (read the JSON array)
 *   - Show status per pipeline (read the last entry's level)
 *
...
impl WorkflowLog {
    /// Create a new workflow log for a pipeline run.
    ///
    /// The log file is written to `output/logs/<pipeline_name>/<run_id>.json`
...
/// List all run log files for a pipeline, sorted by filename (newest last
...
mod tests {
    ...
    async fn test_workflow_log_creates_file() {
        ...
        assert_eq!(parsed.header.pipeline_name, "my-pipeline");
    ...
    async fn test_workflow_log_file_path_structure() {
        let dir = TempDir::new().unwrap();
        let log = WorkflowLog::new(dir.path(), "bug-autofix", "abc-123")
            .await
            .unwrap();

        let expected = dir.path().join("output/logs/bug-autofix/abc-123.json");
    ...
    async fn test_read_run_log_roundtrip() {
        ...
        let parsed = read_run_log(log.path()).await.unwrap();
        assert_eq!(parsed.header.pipeline_name, "test-pipe");
