...
use crate::workflows::stores::{PrReviewResult, PrResponseResult};
use crate::workflows::workflow_log::WorkflowLog;
use crate::workflows::workflow::ParsedTask;
use crate::workflows::github_prs::{GitHubPR, InlineReviewComment, PRComment};
use crate::types::config::WorkflowConfig;
use crate::techniques::critique_loop::CritiqueLoopResult;

/// All resolved prompt template paths/strings, threaded through the workflow.
///
...
/// Unaddressed review comments for the PR response workflow.
#[derive(Debug, Clone, Default)]
...
pub struct StageContext {
    /// Workflow-level configuration (immutable for the run's lifetime).
    ...
    pub logger: Logger,
    /// UUID string identifying this workflow run.
    pub run_id: String,
    ...
    // ── PR response mode (githubPRResponses workflow) ─────────────────────────
    ...
    /// Per-run log file written to `output/logs/<workflow>/<run_id>.json`.
    ...
    // ── Abort signal ─────────────────────────────────────────────────────────
    /// Shared flag — any stage can set this to `true` to stop the workflow.
